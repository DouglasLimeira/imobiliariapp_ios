...
class ListaProntosVC: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {
    
    @IBOutlet weak var collectionView: UICollectionView!

    var imoveisProntos = [Imovel]()
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if (segue.identifier == "detalheImovel"){
            ...
            view.imovel = imoveisProntos[(collectionView.indexPathsForSelectedItems()?.first?.row)!]
        }
    }
    
    func carregaListaProntos(){
        Alamofire.request(.GET, "http://localhost:8080/imoveis/service/imoveis/selectImoveisProntos/\(params.tipoImovel)/\(params.bairro)/\(params.quartoMenor)/\(params.quartoMaior)/\(params.garagemMenor)/\(params.garagemMaior)/\(params.areaMenor)/\(params.areaMaior)/\(params.valorMenor)/\(params.valorMaior)")
            .responseJSON { response in
                if let JSON = response.result.value {
                    ...
                    for imovelJSON in arrayJSON{
                        ...
                        self.imoveisProntos.append(imovel)
                    }
